...
"""Run the fixed V48 terminal-U400 strict1607 diagnostic."""
...
_BASE_SPEC = importlib.util.spec_from_file_location(
    "_pivot_stageb_split_fpr_active_set_probe_evaluation_base", _BASE_PATH
)
...
FORMAL_CONFIG = REPO_ROOT / (
    ...
    "split_fpr_active_set_20260801.py"
)
...
CONTROLLER_IMPORT = (
    "run_stageb_confidence_adapter_candidate_split_fpr_active_set_"
    "probe_evaluation"
)
...
def _v48_postflight(
    ...
    if (
        ...
        raise ProbeEvaluationError(
            "V48 postflight lacks the inherited terminal-U400 diagnostic contract"
        )
    contracts = dict(contracts)
    del contracts["v47_split_boundary_routing_v29"]
    contracts.update(
        {
            "v48_split_fpr_active_set_v30": True,
            "exact_fpr95_active_set_mean_v1": True,
...
_BASE.postflight = _v48_postflight
_CORE.postflight = _v48_postflight


def _formal_main_admission_is_wired(path: Path | None = None) -> bool:
    """Prove that main's admission dispatcher owns an exact V48 branch."""
...
def postflight(
    ...
    return _v48_postflight(preflight_report, summary_path=summary_path)
...
def verify_admission_report(
    ...
    if not _formal_main_admission_is_wired():
        raise ProbeEvaluationError(
            "V48 strict1607 evidence cannot promote formal training until "
            "main._bind_stage_b_confidence_probe_admission has the exact V48 "
            "active-set contract and this controller verifier"
        )
