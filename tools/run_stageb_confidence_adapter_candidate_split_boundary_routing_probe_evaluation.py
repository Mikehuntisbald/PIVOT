...
"""Run the fixed V47 terminal-U400 strict1607 diagnostic."""
...
_BASE_PATH = REPO_ROOT / (
    "tools/run_stageb_confidence_adapter_candidate_split_positive_tail_"
    "probe_evaluation.py"
)
_BASE_SPEC = importlib.util.spec_from_file_location(
    "_pivot_stageb_split_boundary_routing_probe_evaluation_base", _BASE_PATH
)
...
CONTROLLER_IMPORT = (
    "run_stageb_confidence_adapter_candidate_split_boundary_routing_"
    "probe_evaluation"
)
...
_BASE_POSTFLIGHT = _CORE.postflight
...
def _v47_postflight(
    ...
    if (
        ...
        or contracts.get("v46_split_positive_tail_v28") is not True
    ):
        raise ProbeEvaluationError(
            "V47 postflight lacks the inherited terminal-U400 diagnostic contract"
        )
    contracts = dict(contracts)
    del contracts["v46_split_positive_tail_v28"]
    contracts.update(
        {
            "v47_split_boundary_routing_v29": True,
...
_BASE.postflight = _v47_postflight
_CORE.postflight = _v47_postflight


def _formal_main_admission_is_wired(path: Path | None = None) -> bool:
    """Prove that main's admission dispatcher owns an exact V47 branch."""
...
def postflight(
    ...
    return _v47_postflight(preflight_report, summary_path=summary_path)
...
def verify_admission_report(
    ...
    if not _formal_main_admission_is_wired():
        raise ProbeEvaluationError(
            "V47 strict1607 evidence cannot promote formal training until "
            "main._bind_stage_b_confidence_probe_admission has the exact V47 "
            "head/reduction contract and this controller verifier"
        )
