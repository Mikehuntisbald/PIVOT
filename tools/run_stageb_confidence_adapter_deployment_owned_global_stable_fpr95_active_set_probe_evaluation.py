...
"""Run the fixed V58 terminal-U400 strict1607 diagnostic."""
...
_SPEC = importlib.util.spec_from_file_location(
    "_pivot_stageb_deployment_owned_stable_fpr95_active_set_evaluation_base",
    _BASE_PATH,
)
...
SCHEMA = (
    ...
    "stable_fpr95_active_set_probe_evaluation/v1"
)
POSTFLIGHT_SCHEMA = (
    "pivot.stageb.confidence_adapter_deployment_owned_global_"
    "stable_fpr95_active_set_probe_evaluation_postflight/v1"
)
ADMISSION_SCHEMA = (
    "pivot.stageb.confidence_adapter_deployment_owned_global_"
    "stable_fpr95_active_set_formal_admission/v1"
)
...
FORMAL_CONFIG = REPO_ROOT / (
    ...
    "stable_fpr95_active_set_20260802.py"
)
...
OUTPUT = REPO_ROOT / (
    ...
    "highmem_20260802/probe_evaluation/u000400_strict1607"
)
...
CONTROLLER_IMPORT = (
    "run_stageb_confidence_adapter_deployment_owned_global_"
    "stable_fpr95_active_set_probe_evaluation"
)
...
_BASE.postflight = _v58_postflight
_CORE.postflight = _v58_postflight
...
def postflight(
    ...
    return _v58_postflight(preflight_report, summary_path=summary_path)
...
def verify_admission_report(path: Path | None = None, *, health_audit=None):
    if not _formal_main_admission_is_wired():
        raise ProbeEvaluationError(
            "V58 evidence cannot promote formal training until main has its "
            "exact stable active-set binding"
        )
