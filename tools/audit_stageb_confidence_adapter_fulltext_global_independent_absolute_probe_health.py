...
"""Fail-closed health audit for the V55 terminal U400 confidence probe."""
...
_SPEC = importlib.util.spec_from_file_location(
    "_pivot_stageb_fulltext_global_independent_absolute_probe_health_base",
    _BASE_PATH,
)
...
TRAINING_CONTRACT_SCHEMA = "pivot.stageb.dense_duty_training_contract/v37"
EXPECTED_REVISION = (
...
EXPECTED_CONFIG_ENTRY = (
    ...
    "independent_absolute_probe_u0400_20260802.py"
)
EXPECTED_SOURCE_UPDATES = 6551
EXPECTED_UPDATES = 400
LOG_PATH = Path(training.OUTPUT) / "log.txt"

# V55 changes routing semantics while retaining V53/V54's exact two-owner
# parameter and optimizer surface.
...
MIGRATION_SCHEMA = FULLTEXT_GLOBAL_INDEPENDENT_ABSOLUTE_MIGRATION_SCHEMA
...
# The inherited V53 functions resolve these names dynamically. The module is
# isolated above, so the V53 controller and any V54 process remain untouched.
_V55_OVERRIDES = {
    "SCHEMA": SCHEMA,
...
_CORE._default_output = lambda: Path(training.OUTPUT).parent / (
    "u000400_fulltext_global_independent_absolute_health_audit.json"
)


def _audit_v55_migration(args: Mapping[str, Any]) -> dict[str, Any]:
...
def audit() -> dict[str, Any]:
    if MIGRATION_FINGERPRINT_IS_PLACEHOLDER:
        raise ProbeHealthEvidenceError(
            "V55 migration fresh-state SHA256 is still the all-zero placeholder"
        )
