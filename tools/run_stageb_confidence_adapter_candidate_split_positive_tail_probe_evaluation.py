...
"""Run the fixed V46 terminal-U400 strict1607 diagnostic."""
...
_BASE_SPEC = importlib.util.spec_from_file_location(
    "_pivot_stageb_split_positive_tail_probe_evaluation_base", _BASE_PATH
)
...
EXPECTED_ROUTING_REDUCTION = "balanced_mean_v1"
...
CONTROLLER_IMPORT = (
    "run_stageb_confidence_adapter_candidate_split_positive_tail_probe_evaluation"
)
...
def _v46_postflight(
    ...
    if (
        ...
        raise _BASE.ProbeEvaluationError(
            "V46 postflight lacks the inherited terminal diagnostic contract"
        )
    ...
    contracts.update(
        {
            ...
            "v46_split_positive_tail_v28": True,
...
_BASE.postflight = _v46_postflight


def _formal_main_admission_is_wired(path: Path | None = None) -> bool:
    """Prove that main's admission dispatcher owns an exact V46 branch."""
...
def verify_admission_report(
    ...
    if not _formal_main_admission_is_wired():
        raise _BASE.ProbeEvaluationError(
            "V46 strict1607 evidence cannot promote formal training until "
            "main._bind_stage_b_confidence_probe_admission has the exact V46 "
