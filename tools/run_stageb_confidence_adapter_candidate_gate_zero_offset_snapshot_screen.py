...
"""Run diagnostic-only strict1607 screens for the archived v39 snapshots."""
...
SNAPSHOTS = dict(combined_eval._V39_IMMUTABLE_ARCHIVED_SNAPSHOT_PATHS)
OUTPUTS = dict(combined_eval._V39_IMMUTABLE_ARCHIVED_SCREEN_OUTPUTS)
LOGS = {
    update: combined_eval._V39_IMMUTABLE_ARCHIVED_SCREEN_ROOT
...
def _expected_sha256(update: int) -> str:
    return str(ref_eval._V39_IMMUTABLE_ARCHIVED_SNAPSHOTS[update]["sha256"])


def inspect(update: int) -> Dict[str, Any]:
    records = ref_eval._verify_v39_immutable_archived_diagnostic_files(
        SNAPSHOTS[update]
    )
    return {
        "schema": "pivot.stageb.v39_immutable_snapshot_screen/v1",
...
def verify_summary(update: int) -> Dict[str, Any]:
    ...
    if (
        ...
        or row.get("immutable_v39_archived_snapshot_diagnostic") is not True
    ...
    if (
        ...
        != "pivot.stageb.v39_immutable_archived_diagnostic/v1"
    ...
    terminal_sha256 = str(
        ref_eval._V39_IMMUTABLE_ARCHIVED_TERMINAL["sha256"]
    )
    for key in (
