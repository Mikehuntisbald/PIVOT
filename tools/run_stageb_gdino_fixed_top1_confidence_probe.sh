...
usage() {
    cat <<'EOF'
Usage: tools/run_stageb_gdino_fixed_top1_confidence_probe.sh [options]

Options:
  --source-kind KIND       Must be rank (default: rank)
  --source-checkpoint PATH Audited R milestone used only with pretrain_model_path
  --source-audit PATH      Audit sidecar for the selected R milestone
  --output-root PATH       Fixed-top1 probe output root
  --max-target N|auto      Last milestone; auto uses the sealed train partition (default: auto)
  --partition-audit PATH   Sealed partition used by auto max-target
  --continue               Continue an interrupted fixed-top1-scope run
  ...
  --master-port PORT       DDP master port (default: 29531)
EOF
}
...
SOURCE_CHECKPOINT=""
SOURCE_AUDIT=""
OUTPUT_ROOT="outputs/stageb_gdino_adapter_fixed_top1_confidence_probe_20260712"
MAX_TARGET=auto
...
case "${SOURCE_KIND}" in
    rank) ;;
    *) echo "[FAIL] --source-kind must be rank" >&2; exit 2 ;;
esac
if [[ "${MAX_TARGET}" == "auto" ]]; then
    if [[ ! -f "${PARTITION_AUDIT}" ]]; then
        echo "[FAIL] auto max-target requires the partition audit: ${PARTITION_AUDIT}" >&2
        exit 2
    fi
    MAX_TARGET="$("${PYTHON_BIN}" -c \
        'import json,sys; value=json.load(open(sys.argv[1], encoding="utf-8")).get("recommended_max_target"); assert type(value) is int; print(value)' \
        "${PARTITION_AUDIT}")" || {
            echo "[FAIL] partition has no selection-ready recommended max target" >&2
            exit 2
        }
fi
case "${MAX_TARGET}" in
    50|100|250|500|1000) ;;
    *) echo "[FAIL] --max-target must be auto, 50, 100, 250, 500, or 1000" >&2; exit 2 ;;
esac
...
if [[ "${DRY_RUN}" == "1" ]]; then
    PYTHONPATH="${REPO_ROOT}${PYTHONPATH:+:${PYTHONPATH}}" \
        "${PYTHON_BIN}" "${AUDITOR}" static --max-target "${MAX_TARGET}" >/dev/null
    echo "[OK] fixed-top1 data/config/code static audit passed"
...
if [[ -f "${PREFLIGHT}" ]]; then
    ...
    echo "[FAIL] fresh fixed-top1 output directory is not empty: ${OUTPUT_ROOT}" >&2
    exit 2
fi
...
for target in "${MILESTONES[@]}"; do
    ...
    if [[ -f "${snapshot}" || -f "${audit}" ]]; then
        if [[ ! -f "${snapshot}" || ! -f "${audit}" ]]; then
            echo "[FAIL] incomplete preserved fixed-top1 milestone ${target}" >&2
            exit 2
        fi
    ...
    if [[ "${live_iteration}" -gt "${previous_iteration}" ]]; then
        if [[ "${CONTINUE_RUN}" != "1" ]]; then
            echo "[FAIL] found unpreserved fixed-top1 live state; inspect and pass --continue" >&2
            exit 2
        fi
        ...
        if [[ "${#lineage_files[@]}" -lt 1 ]]; then
            echo "[FAIL] live fixed-top1 checkpoint has no recorded segment ancestry" >&2
            exit 2
        fi
    ...
    echo "[RUN] fixed-top1 target=${target} mode=${initialization_mode} source=${source_checkpoint}"
    print_command "${TRAIN_COMMAND[@]}"
    ...
    if [[ ! -f "${LIVE}" ]]; then
        echo "[FAIL] fixed-top1 target ${target} produced no checkpoint_iter.pth" >&2
        exit 2
    fi
    live_iteration="$("${PYTHON_BIN}" "${AUDITOR}" metadata --checkpoint "${LIVE}" --field iteration)"
    if [[ "${live_iteration}" -ne "${target}" ]]; then
        echo "[INCOMPLETE] fixed-top1 run stopped at ${live_iteration}; rerun with --continue" >&2
        exit 3
    fi
    cp --reflink=auto --preserve=timestamps "${LIVE}" "${snapshot}"
    milestone_command=(
...
echo "[OK] completed fixed-top1 confidence milestones: ${OUTPUT_ROOT}/milestones"
