...
"""Run the fixed V56 terminal-U400 strict1607 diagnostic."""
...
_BASE_PATH = REPO_ROOT / (
    "tools/run_stageb_confidence_adapter_fulltext_global_independent_"
    "absolute_probe_evaluation.py"
)
_SPEC = importlib.util.spec_from_file_location(
    "_pivot_stageb_deployment_owned_global_probe_evaluation_base", _BASE_PATH
)
...
SCHEMA = "pivot.stageb.confidence_adapter_deployment_owned_global_probe_evaluation/v1"
POSTFLIGHT_SCHEMA = (
    "pivot.stageb.confidence_adapter_deployment_owned_global_"
    "probe_evaluation_postflight/v1"
)
ADMISSION_SCHEMA = (
    "pivot.stageb.confidence_adapter_deployment_owned_global_formal_admission/v1"
)
...
EXPECTED_HEAD_CONTRACT = "split_token_veto_deployment_owned_global_absolute_v9"
...
EXPECTED_TRAINABLE_PARAMETERS = 468_164
...
for _module in (_V55, _V54, _V53, _BASE, _CORE):
...
_BASE_POSTFLIGHT = _V55._v55_postflight
...
def _v56_postflight(
    ...
    if not isinstance(contracts, Mapping) or contracts.get(
        ...
        raise ProbeEvaluationError("V56 postflight lacks the inherited deployed-score contract")
    contracts = dict(contracts)
    for stale in (
        "v55_rank_full_expression_global_independent_absolute_v37",
        "same_v53_v54_data_loss_update_parameter_surface",
        "local_candidate_logits_excluded_from_deployed_global",
        "local_candidate_and_global_pool_logits_are_distinct",
    ):
    ...
    contracts.update(
        {
            ...
            "deployed_global_trunk_owned_only_by_deployed_losses": True,
            "candidate_head_input_is_dense_global_detached": True,
...
_BASE.postflight = _v56_postflight
_CORE.postflight = _v56_postflight
...
def postflight(
    ...
    return _v56_postflight(preflight_report, summary_path=summary_path)
...
def verify_admission_report(
    ...
    if not _formal_main_admission_is_wired():
        raise ProbeEvaluationError(
            "V56 strict1607 evidence cannot promote formal training until main "
            "has the exact deployment-owned contract and this verifier"
        )
