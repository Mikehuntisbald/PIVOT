...
"""Run the fixed V51 terminal-U400 strict1607 diagnostic."""
...
_BASE_SPEC = importlib.util.spec_from_file_location(
    "_pivot_stageb_split_independent_deployed_router_probe_evaluation_base",
    _BASE_PATH,
)
...
SCHEMA = (
    ...
    "router_probe_evaluation/v1"
)
POSTFLIGHT_SCHEMA = (
    "pivot.stageb.confidence_adapter_candidate_split_independent_deployed_"
    "router_probe_evaluation_postflight/v1"
)
ADMISSION_SCHEMA = (
    "pivot.stageb.confidence_adapter_candidate_split_independent_deployed_"
    "router_formal_admission/v1"
)
...
CONTROLLER_IMPORT = (
    "run_stageb_confidence_adapter_candidate_split_independent_deployed_router_"
    "probe_evaluation"
)
...
def _v51_postflight(
    ...
    if (
        ...
        raise ProbeEvaluationError(
            "V51 postflight lacks the inherited terminal-U400 diagnostic contract"
        )
    contracts = dict(contracts)
    del contracts["v47_split_boundary_routing_v29"]
    contracts.update(
        {
            "v51_split_independent_deployed_router_v33": True,
            "routing_weight_0_1": True,
            "balanced_top_quarter_cvar_v2": True,
            "top_quarter_cvar_v2": True,
            "all_mean_v1": True,
            "target_iou_v1": True,
            "split_token_veto_deployed_router_global_absolute_v5": True,
...
_BASE.postflight = _v51_postflight
_CORE.postflight = _v51_postflight
...
def _formal_main_admission_is_wired(path: Path | None = None) -> bool:
    """Prove that one V51 branch owns every exact promotion condition."""
...
def postflight(
    ...
    return _v51_postflight(preflight_report, summary_path=summary_path)
...
def verify_admission_report(
    ...
    if not _formal_main_admission_is_wired():
        raise ProbeEvaluationError(
            "V51 strict1607 evidence cannot promote formal training until "
            "main._bind_stage_b_confidence_probe_admission has the exact V51 "
            "head/weight/reduction contract and this controller verifier"
        )
