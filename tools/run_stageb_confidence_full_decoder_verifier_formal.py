...
"""Run fresh V61 formal confidence training after strict1607 admission."""
...
_SPEC = importlib.util.spec_from_file_location(
    "_pivot_stageb_full_decoder_verifier_formal_base", _BASE_PATH
)
...
OUTPUT = REPO_ROOT / (
    ...
    "dense_duty_full_decoder_confidence_verifier_20260803/formal/confidence"
)
CHECKPOINT = OUTPUT / "checkpoint_iter.pth"
LOCK = OUTPUT.parent.parent / ".full_decoder_verifier_formal.lock"
...
def verify_probe_admission():
    from tools import run_stageb_confidence_full_decoder_verifier_probe_evaluation as promotion

    if promotion.FORMAL_CONFIG.resolve(strict=True) != CONFIG.resolve(strict=True):
        raise _BASE.ControllerError("probe admission promotes a different V61 config")
