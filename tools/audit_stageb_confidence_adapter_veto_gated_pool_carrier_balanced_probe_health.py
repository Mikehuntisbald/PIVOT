...
"""Audit U300 health for carrier-balanced word-veto confidence."""
...
from tools import run_stageb_confidence_adapter_veto_gated_pool_carrier_balanced_probe as probe  # noqa: E402


_BASE_PATH = REPO_ROOT / "tools/audit_stageb_confidence_adapter_veto_gate_probe_health.py"
_SPEC = importlib.util.spec_from_file_location(
    "_pivot_stageb_confidence_adapter_carrier_balanced_health_base", _BASE_PATH
)
...
SCHEMA = "pivot.stageb.confidence_adapter_veto_gated_pool_probe_health/v7"
_FIELDS = (
...
def _check_close(value: float, expected: float, *, atol: float = 1e-4):
    return _BASE._check(value, f"abs(value - {expected}) <= {atol}", abs(value - expected) <= atol)
    ...
    checks.update(
        {
            ...
            "u222_tn_balance_exact": _check_close(
                tn_balanced, 0.5 * (tn_all + tn_carrier)
            ),
