...
"""Run the fixed V45 terminal-U400 strict1607 diagnostic.

The strict1607 result remains diagnostic-only. ``verify_admission_report``
fails closed until ``main._bind_stage_b_confidence_probe_admission`` contains
the exact V45 contract and imports this controller as its promotion verifier.
"""
...
_BASE_SPEC = importlib.util.spec_from_file_location(
    "_pivot_stageb_split_tail_aligned_probe_evaluation_base", _BASE_PATH
)
...
FORMAL_CONFIG = REPO_ROOT / (
    ...
    "split_tail_aligned_20260801.py"
)
...
CONTROLLER_IMPORT = (
    "run_stageb_confidence_adapter_candidate_split_tail_aligned_probe_evaluation"
)
...
def _v45_postflight(
    ...
    if (
        ...
        raise _BASE.ProbeEvaluationError(
            "V45 postflight lacks the inherited terminal diagnostic contract"
        )
    ...
    contracts.update(
        {
            ...
            "v45_split_tail_aligned_v27": True,
            "split_token_veto_global_absolute_joint_clip_v3": True,
...
_BASE.postflight = _v45_postflight


def _formal_main_admission_is_wired(path: Path | None = None) -> bool:
    """Prove that main's admission dispatcher owns an exact V45 branch."""
...
def verify_admission_report(
    ...
    if not _formal_main_admission_is_wired():
        raise _BASE.ProbeEvaluationError(
            "V45 strict1607 evidence cannot promote formal training until "
            "main._bind_stage_b_confidence_probe_admission has the exact V45 "
