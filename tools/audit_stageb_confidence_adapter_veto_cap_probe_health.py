...
"""Audit U300 health for the v4 coverage-gated absolute cap."""
...
from tools import run_stageb_confidence_adapter_veto_cap_probe as probe  # noqa: E402


_BASE_PATH = REPO_ROOT / "tools/audit_stageb_confidence_adapter_veto_gate_probe_health.py"
_SPEC = importlib.util.spec_from_file_location(
    "_pivot_stageb_confidence_adapter_veto_cap_health_base", _BASE_PATH
)
...
SCHEMA = "pivot.stageb.confidence_adapter_veto_cap_probe_health/v4"
_CAP_FIELDS = (
...
_BASE.probe = probe
_BASE.SCHEMA = SCHEMA
_BASE.REQUIRED_U222_FIELDS = _BASE.REQUIRED_U222_FIELDS + _CAP_FIELDS
...
def _health_checks(
    ...
    checks.update(
        {
            "u222_positive_veto_coverage_closed": _BASE._check(
                positive_coverage, "<= 0.10", positive_coverage <= 0.10
            ),
            "u222_tn_veto_coverage_open": _BASE._check(
                tn_coverage, ">= 0.80", tn_coverage >= 0.80
            ),
            "u222_positive_sample_gate_closed": _BASE._check(
                positive_gate, "<= 0.05", positive_gate <= 0.05
            ),
            "u222_tn_sample_gate_open": _BASE._check(
