...
"""Run the fixed V52 terminal-U400 strict1607 diagnostic."""
...
_BASE_PATH = REPO_ROOT / (
    "tools/run_stageb_confidence_adapter_candidate_split_independent_"
    "deployed_router_probe_evaluation.py"
)
_SPEC = importlib.util.spec_from_file_location(
    "_pivot_stageb_candidate_sample_calibrator_probe_evaluation_base", _BASE_PATH
)
...
SCHEMA = "pivot.stageb.confidence_adapter_candidate_sample_calibrator_probe_evaluation/v1"
POSTFLIGHT_SCHEMA = (
    "pivot.stageb.confidence_adapter_candidate_sample_calibrator_"
    "probe_evaluation_postflight/v1"
)
ADMISSION_SCHEMA = (
    "pivot.stageb.confidence_adapter_candidate_sample_calibrator_formal_admission/v1"
)
...
_BASE_POSTFLIGHT = _V51.postflight
...
def _v52_postflight(
    ...
    if (
        ...
        or contracts.get("v51_split_independent_deployed_router_v33") is not True
    ):
        raise ProbeEvaluationError(
            "V52 postflight lacks the inherited terminal-U400 diagnostic contract"
        )
    contracts = dict(contracts)
    del contracts["v51_split_independent_deployed_router_v33"]
    contracts.update(
        {
...
_BASE.postflight = _v52_postflight
_CORE.postflight = _v52_postflight
...
def postflight(
    ...
    return _v52_postflight(preflight_report, summary_path=summary_path)
...
def verify_admission_report(
    ...
    if not _formal_main_admission_is_wired():
        raise ProbeEvaluationError(
            "V52 strict1607 evidence cannot promote formal training until main "
            "has the exact V52 head/weight/reduction contract and verifier"
        )
