#!/usr/bin/env python3
"""Train one locked seed of matched confidence readouts; never score validation.
...
from tools.confidence_readout import (
    LOCALIZERS, load_readout_cache, make_readout_heads, native_labels,
    parse_arm, readout_scores, training_arms,
)
...
def run(args):
    ...
    design = {"schema": "arrow.confidence_readout.training_lock/v1",
    ...
    with _deterministic_algorithms():
        ...
        if initialpath.exists():
            ...
            _atomic_torch(initialpath, {"schema": "arrow.confidence_readout.initial/v1", "design": lock_binding,
        ...
        for epoch in range(start_epoch + 1, 6):
            ...
            for task, indices in events:
                ...
                pos, neg = [p["positive"] for p in batch], [p["negative"] for p in batch]
                rows = pos + neg
                ...
                for arm, model in models.items():
                    readout, target = parse_arm(arm)
                    loss, grad = train_step(model, opts[arm], features, native, mask, selected, correct,
                                            target=target, readout=readout, positive_count=len(pos))
                    loss_sums[arm] += loss
                ...
                if updates % 100 == 0:
                    print(f"[READOUT] seed={args.seed} localizer={args.localizer} epoch={epoch} "
            ...
            _atomic_torch(ckpath, {"schema": "arrow.confidence_readout.head_checkpoint/v1",
    ...
    for artifact in (*design["code"].values(), *caches.values(), trunk, design["study_protocol"]):
        verify(artifact)
    _atomic_json(root / "postflight.json", {"schema": "arrow.confidence_readout.training_postflight/v1",
        ...
        "schedule_factory": "tools.train_finecops_bce_l2_heads._epoch_schedule; rank events generated then skipped",
        "no_optimizer_skips": True, "no_amp": True, "frozen_native_rank": "bitwise unchanged",
        "validation_model_evaluations": 0, "test_forwards": 0, "gref_forwards": 0})
    print(f"[READOUT] COMPLETE seed={args.seed} localizer={args.localizer}", flush=True)
