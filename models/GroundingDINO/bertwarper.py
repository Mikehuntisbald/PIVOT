...
class BertModelWarper(nn.Module):
    ...
    def get_head_mask(self, head_mask, num_hidden_layers):
        """Compatibility shim for Transformers versions where BertModel no longer exposes get_head_mask."""
        if head_mask is None:
            return [None] * num_hidden_layers

        if not isinstance(head_mask, torch.Tensor):
            head_mask = torch.tensor(head_mask, dtype=self.embeddings.word_embeddings.weight.dtype)
        else:
