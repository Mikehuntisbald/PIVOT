...
# 这里按你的实际文件路径改
from bertwarper import BertModelWarper  # from xxx import BertModelWarper
...
class VGTextDataset(Dataset):
    ...
    这里会在 phrase 最后补一个 '.'，保持和你其他 text encoder 输入风格统一。
    """
    ...
    def __init__(
        ...
        # 读 canonical：用来过滤合法 class_id，顺便存个 id->name 做 debug
...
class FocalLoss(nn.Module):
    ...
    这里 alpha 默认为 None，不做 class re-weight。
    """

    def __init__(self, gamma=2.0, alpha=None, reduction="mean"):
        super().__init__()
        self.gamma = gamma
        self.alpha = alpha  # 可以是 None / scalar / (C,) tensor
...
class MultiClassPhraseClassifier(nn.Module):
    ...
    使用 BertModelWarper 做 text encoder：
      ...
      - pooled_output 走一个 Dropout + Linear -> num_classes logits
    """
...
def evaluate(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            loss = ce(logits, labels)  # 验证时用标准 CE 统计个指标就够了
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        save_path = out_dir / f"multiclass_bertwarper_focal_epoch{epoch+1}.pt"
