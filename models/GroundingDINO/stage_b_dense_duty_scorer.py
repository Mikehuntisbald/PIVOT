...
class TokenAwareConfidenceAdapter(nn.Module):
        ...
        if self.head_gradient_contract in {
            ...
            return self._merge_parameters(
                self._merge_parameters(
                    self.deployed_global_trunk_parameters(), query_head
                ),
    ...
    def _assert_head_parameter_ownership(self) -> None:
        ...
        active_complete = complete - diagnostic if frozen_diagnostic else complete
        ...
        *,
        ...
        if (
            ...
            if self.head_gradient_contract == (
                ...
                # V59 promotes this zero-initialized query evidence into the
        ...
        for layer in range(int(confidence_base_flat.shape[0])):
            ...
            if fulltext_global_absolute_enabled:
                if patch_softmin_veto_only_enabled:
                    ...
                    # A normalized rank-conditioned log-sum-exp is monotone in
                    # every deployed query logit and exactly zero when the
                    # zero-initialized query head emits zero. Frozen rank only
                    # supplies query preference; it is never an additive
                    # confidence coordinate and receives no gradient.
                    temperature = float(self.confidence_pool.pool_temperature)
                    frozen_reference = (
                        confidence_reference_base_flat[layer].detach().float()
                    )
                    query_absolute = confidence_base_flat[layer].float()
                    masked_reference = frozen_reference.masked_fill(
                        ~flat_eligible, -torch.inf
                    )
                    query_global_logit = temperature * (
                        torch.logsumexp(
                            (masked_reference + query_absolute) / temperature,
                            dim=1,
                        )
                        - torch.logsumexp(
                            masked_reference / temperature,
                            dim=1,
                        )
                    )
                    query_global_logit = query_global_logit.masked_fill(
                        ~flat_valid, 0.0
                    )
                    global_layers.append(query_global_logit + residual)
                elif independent_global_absolute_enabled:
