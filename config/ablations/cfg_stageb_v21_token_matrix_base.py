...
# Shared contract for Table C. Every row uses the v19 explicit
# base-plus-gate confidence architecture plus the v20 acc50-aligned rank hard
# negatives. Individual row configs may change only the declared token-role
# knobs and the complementary predicate-pair rank weight.
skip_eval = True
...
# Table C uses proposal-covered D3 rows for a scope-labelled confidence
# diagnostic while retaining certified single-edit token supervision. It never
# upgrades these rows to image-global TN.
