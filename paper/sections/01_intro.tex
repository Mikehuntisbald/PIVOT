...
\begin{figure*}[t]
  ...
  exposes an Admission failure rather than hiding it.}
  \label{fig:teaser}
\end{figure*}
...
Across FineCops-Ref~\cite{liu2024finecops} and a restricted single/no-target
gRefCOCO slice, the isolated rejection score improves relative discrimination,
while a threshold sealed internally does not preserve 95\% TPR externally.
...
\begin{enumerate}[leftmargin=*,itemsep=1pt,topsep=2pt]
  ...
  external negative constructions expose both transferable ordering and the
  boundary of fixed-threshold calibration.
\end{enumerate}
