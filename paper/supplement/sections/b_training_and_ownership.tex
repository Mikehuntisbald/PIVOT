...
\input{supplement/tables/supp_arrow_capacity_control}
...
The pretrained-trunk replay in Tab.~\ref{tab:supp-pretrain-ownership} removes
RefCOCO-specific full-model trunk fine-tuning while preserving the same
capacity-matched owner architectures and fixed Ranking/Abstention head
supervision.  Its Shared-Wide gradients are near orthogonal on average at U150 but retain a
moderate negative tail.  Shared-Wide is slightly better on both pooled REC
surfaces, while the Strict-TN2031 interval crosses zero and points in the same
direction.  This confirms that broad pretrained representation quality, not
only task-specific trunk adaptation, can make wide sharing effective; it does
not make the experiment image-disjoint or unsupervised.

The direct-parent replay in Tab.~\ref{tab:supp-original-parent-ownership}
freezes the immediate pure-GDINO parent of the frozen-base checkpoint and
applies the same raw-query owners used in the MM-GDINO blocks.  Shared-Wide is
substantially better on both pooled REC surfaces; Isolated fails
non-inferiority, while the rejection interval crosses zero.

Tab.~\ref{tab:supp-same-head-stage-axis} closes the earlier head mismatch by
replaying those exact 100k owners on the adapted descendant.  The two effective
938-tensor architectures match; only 727 trunk tensors change.  Initialization,
schedule bytes, losses, seeds, optimizer states, scorer, and evaluator are
identical.  Adaptation narrows the isolation Test5 penalty from 2.001 to 0.053 points;
the paired difference-in-differences excludes zero.  Its FPR95 shift is
directionally favorable but unresolved.  This supports an adaptation-dependent
ownership effect rather than a universal hard-isolation rule.

\input{supplement/tables/supp_training_controls}
