...
Admission then updates only its category-eligibility owner and a training-only
auxiliary carrier.  Rejection subsequently updates only its sample-level
...
\begin{equation}
 \begin{aligned}
 \mu&={1\over n}\sum_{i\in\mathcal M}a_i,\\
 \sigma_a&=\sqrt{\max\!\left(
 {1\over n}\sum_{i\in\mathcal M}(a_i-\mu)^2,10^{-6}\right)},\\
 \tilde a_i&=\operatorname{clip}((a_i-\mu)/\sigma_a,-5,5).
 \end{aligned}
 \label{eq:supp-admission-standardize}
\end{equation}
The training-only carrier is a LayerNorm--MLP $3\!\to\!64\!\to\!64\!\to\!1$
...
\begin{equation}
 ...
 \ell_k&=T_A\operatorname{softplus}
       \left({m_A-p_k+\max_{i\in\mathcal N}a_i\over T_A}\right).
 \end{aligned}
...
one, and $(T_A,m_A)=(0.1,0.1)$.  At inference, the carrier is discarded and
the eligible set is computed solely from $\tilde a$ with margin three.
...
\begin{equation*}
 \begin{aligned}
 [\max s,\ &\operatorname{mean}(\operatorname{top3}s),
   \operatorname{mean}s,\\
...
\begin{align}
 ...
 \mathcal L_-&={1\over B}\sum_j0.1\,
 \operatorname{softplus}\!\left({c_j^- -\hat\tau\over0.1}\right),\\
 \mathcal L_+&={1\over B}\sum_j[\tau_Q-0.02-c_j^+]_+,\\
...
to Ranking or Rejection owners, or from a Rejection loss to Admission or
...
additional phased schedule is retained as a scheduling diagnostic.  The
supported conclusion concerns exclusive ownership; the evidence does not
establish an independent benefit from phasing once ownership is isolated.
...
0.1, and an initial AMP scale of 8192.  Rejection trajectories use 50 updates
