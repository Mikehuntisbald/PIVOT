...
def table2(reg: Registry) -> None:
    ...
    for route, label in (
        ...
        strong_rows.append((
            label,
            f"{int(reg.value(f'strong_ownership.{route}.trainable_parameters')):,}",
            f"{int(reg.value(f'strong_ownership.{route}.macs_per_query_both_outputs')):,}",
            conflict,
            registered_mean_sd(f"strong_ownership.{route}", "testab"),
            registered_mean_sd(f"strong_ownership.{route}", "strict_fpr95"),
        ))
    ...
    contrast_rows = []
    for suffix, label in (
        ("shared_128", "Isolated $-$ Shared-128"),
        ("shared_wide", "Isolated $-$ Shared-Wide"),
        ("native", "Isolated $-$ Native"),
    ):
        rec = reg.item(f"strong_ownership.contrast.isolated_vs_{suffix}.rec_gain")
        fpr = reg.item(f"strong_ownership.contrast.isolated_vs_{suffix}.fpr95_gain")
        p_value = reg.value(f"strong_ownership.contrast.isolated_vs_{suffix}.holm_iut_p")
        contrast_rows.append((
            ...
            f"{p_value:.4f}",
            "pass" if suffix == "native" else "fail",
        ))
