...
\end{{tabular}}
\vspace{{2pt}}
\parbox{{0.98\columnwidth}}{{\scriptsize V/T/N denote visual support, canonical text, and null cues. Percentages; Test5 is mean $\pm$ SD. Visual-support coverage is {coverage}\%; unsupported rows are not counted as correct rejection.}}
\end{{table}}
"""
...
def table4(reg: Registry) -> None:
    ...
    fine_rows.append((
        "MM-GDINO-T",
        pct(reg.value("finecops.official_mm_gdino_t.positive_p1_macro")),
        pct(reg.value("finecops.official_mm_gdino_t.negative_text_recall1")),
        pct(reg.value("finecops.official_mm_gdino_t.negative_image_recall1")),
        pct(reg.value("finecops.official_mm_gdino_t.negative_text_auroc_type_macro")),
        pct(reg.value("finecops.official_mm_gdino_t.negative_image_auroc_type_macro")),
    ))
...
\begin{{table}}[t]
\centering
\caption{{External transfer. FineCops uses full-test text-cue routes; gRefCOCO is the restricted single/no-target slice.}}
\label{{tab:external}}
