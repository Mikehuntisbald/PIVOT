...
def table2(reg: Registry) -> None:
    ...
    internal_rows = [
        (
            "ARROW-U2", "Shared scalar", "--", "--",
            "/".join(f"{value:.2f}" for value in cosines),
            pct(reg.value("ownership.o0.test5")),
            pct(reg.value("ownership.o0.strict2031_fpr95")),
        ),
        (
            "ARROW-U2", "Isolated", "--", "--", "zero",
            pct(reg.value("ownership.o2.test5")),
            pct(reg.value("ownership.o2.strict2031_fpr95")),
        ),
        (
            "ARROW-U2", "Shared-Wide (matched)",
            kparams(reg.value("b58_capacity.shared_wide.score_owner_parameters")),
            f"{reg.value('b58_capacity.shared_wide.score_macs_per_query_and_output') / 1000.0:.1f}",
            "/".join(
                f"{reg.value(f'b58_capacity.shared_wide.seed{seed}.cosine_mean'):+.2f}"
                for seed in (17, 42, 73)
            ),
            pct(reg.value("b58_capacity.shared_wide.test5"), 3),
            pct(reg.value("b58_capacity.shared_wide.strict2031_fpr95"), 3),
        ),
        (
            "ARROW-U2", "Isolated replay (matched)",
            kparams(reg.value("b58_capacity.isolated.score_owner_parameters")),
            f"{reg.value('b58_capacity.isolated.score_macs_per_query_and_output') / 1000.0:.1f}",
            "zero by design",
            pct(reg.value("b58_capacity.isolated.test5"), 3),
            pct(reg.value("b58_capacity.isolated.strict2031_fpr95"), 3),
        ),
    ]
    strong_rows = []
    labels = (
    ...
    for route, label in labels:
        if route.startswith("shared"):
            gradient = "/".join(
                f"{reg.value(f'strong_ownership.{route}.seed{seed}.cosine_mean'):+.3f}"
                for seed in (17, 42, 73)
            )
        elif route == "isolated_128":
            gradient = "zero by design"
        else:
            gradient = "--"
        strong_rows.append((
            "Strong e5",
            label,
            kparams(reg.value(f"strong_ownership.{route}.trainable_parameters")),
            f"{reg.value(f'strong_ownership.{route}.macs_per_query_both_outputs') / 1000.0:.1f}",
            gradient,
            pct(reg.value(f"strong_ownership.{route}.testab"), 3),
            pct(reg.value(f"strong_ownership.{route}.strict_fpr95"), 3),
        ))
    table_body = "\n".join(
        " & ".join(row) + r" \\" for row in (*internal_rows, *strong_rows)
    )
    native_gain = pct(
        reg.value("strong_ownership.contrast.isolated_vs_native.fpr95_gain"), 3
    )
    body = rf"""% Generated by paper/scripts/make_main_tables.py.
\begin{{table*}}[t]
\centering
...
significant.  On strong e5, Isolated improves Native by {native_gain} FPR95
