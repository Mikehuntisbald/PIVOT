...
FORMAL_CONFIG = (
    REPO_ROOT
    / "config/ablations/cfg_stageb_native_patch_category_d4_u200.py"
)
SMOKE_CONFIG = (
    REPO_ROOT
    / "config/ablations/cfg_stageb_native_patch_category_d4_smoke.py"
)
DATASET_CONFIG = (
    REPO_ROOT
...
def _d4_runtime_args(root: Path) -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        stage_b_native_patch_execution_scope=_STAGE_B_NATIVE_PATCH_D4_U200_SCOPE,
        ...
        stage_b_native_patch_d4_base_initializer_path=str(base_initializer),
        stage_b_native_patch_d4_base_initializer_sha256=_sha256(base_initializer),
...
class StageBNativePatchCategoryD4FormalTest(unittest.TestCase):
    def test_formal_and_smoke_configs_are_exact(self) -> None:
        ...
        self.assertEqual(
            ...
            REPO_ROOT
            / "outputs/paper_cvpr_v1/native_patch_category_d4_seed42_s43_b36a2_lr5e5_amp8_u200_v1",
        )
        self.assertEqual(
    ...
    def test_formal_config_binds_actual_locked_inputs(self) -> None:
        ...
        self.assertEqual(
            args.stage_b_native_patch_runtime_binding["initializer"]["sha256"],
            values["stage_b_native_patch_initializer_sha256"],
        )
        self.assertEqual(
            args.stage_b_native_patch_runtime_binding["d4_base_initializer"][
                "sha256"
            ],
            values["stage_b_native_patch_d4_base_initializer_sha256"],
        )

    def test_d4_runtime_binding_is_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            args = _d4_runtime_args(Path(directory))
            ...
            self.assertEqual(
                binding["schema"],
                "pivot.stageb.native_patch_category_d4_runtime/v1",
            )
            self.assertEqual(binding["runtime"]["max_train_iters"], 200)
            self.assertEqual(binding["runtime"]["iter_checkpoint_interval"], 50)
            self.assertIn("d4_base_initializer", binding)
            self.assertNotIn("d3_base_initializer", binding)

            for key, bad_value, message in (
                ("stage_b_data_driven_sampler_seed", 42, "formal contract drifted"),
                ("max_train_iters", 201, "runtime drifted"),
                (
                    "stage_b_native_patch_d4_positive_keep_weight",
                    16.0,
                    "objective drifted",
    ...
    def test_d4_strict_resume_uses_u200_contract_and_d1_lineage(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_pretrain_route_audits_d1_as_d4_source(self) -> None:
        source = inspect.getsource(training_main.main)
        self.assertIn('"d4_positive_protected_critical_winner"', source)
        self.assertIn("stage_b_native_patch_d4_base_initializer_path", source)
        self.assertIn("audit_d2_source_transition", source)
        self.assertIn("stage_b_native_patch_d4_source_audit", source)
        self.assertIn("pivot.stageb.native_patch_category_d4_source_audit/v1", source)
