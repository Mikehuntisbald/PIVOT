...
FORMAL_CONFIG = (
    REPO_ROOT
    / "config/ablations/cfg_stageb_native_patch_category_d8_u100.py"
)
SMOKE_CONFIG = (
    REPO_ROOT
    / "config/ablations/cfg_stageb_native_patch_category_d8_smoke.py"
)
DATASET_CONFIG = (
    REPO_ROOT
...
def _d8_runtime_args(root: Path) -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        stage_b_native_patch_execution_scope=_STAGE_B_NATIVE_PATCH_D8_U100_SCOPE,
        ...
        stage_b_native_patch_d8_base_initializer_path=str(base_initializer),
        stage_b_native_patch_d8_base_initializer_sha256=_sha256(base_initializer),
...
class StageBNativePatchCategoryD8FormalTest(unittest.TestCase):
    def test_formal_and_smoke_configs_are_exact(self) -> None:
        ...
        d8_keys = [key for key in cfg if key.startswith("stage_b_native_patch_d8_")]
        self.assertFalse(
            any(
                forbidden in key
                for key in d8_keys
        ...
        self.assertEqual(
            ...
            REPO_ROOT
            / "outputs/paper_cvpr_v1/native_patch_category_d8_seed42_s43_b36a2_lr5e5_amp8_u100_v1",
        )
        self.assertEqual(
    ...
    def test_formal_config_binds_actual_locked_inputs(self) -> None:
        ...
        self.assertEqual(
            binding["schema"],
            "pivot.stageb.native_patch_category_d8_runtime/v1",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            binding["d8_base_initializer"]["sha256"],
            values["stage_b_native_patch_d8_base_initializer_sha256"],
        )
        self.assertEqual(binding["runtime"]["max_train_iters"], 100)
        self.assertEqual(binding["runtime"]["iter_checkpoint_interval"], 50)

    def test_d8_runtime_binding_is_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            args = _d8_runtime_args(Path(directory))
            allocator = {"PYTORCH_CUDA_ALLOC_CONF": "expandable_segments:True"}
            with patch.dict(os.environ, allocator):
                _bind_stage_b_native_patch_runtime_inputs(args)
            binding = args.stage_b_native_patch_runtime_binding
            self.assertIn("d8_base_initializer", binding)
            self.assertNotIn("d7_base_initializer", binding)
    ...
    def test_d8_strict_resume_uses_u100_contract_and_d1_lineage(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_pretrain_route_audits_d1_as_d8_source(self) -> None:
        source = inspect.getsource(training_main.main)
        self.assertIn('"d8_state_class_macro_anchor"', source)
        self.assertIn("stage_b_native_patch_d8_base_initializer_path", source)
        self.assertIn("audit_d2_source_transition", source)
        self.assertIn("stage_b_native_patch_d8_source_audit", source)
        self.assertIn(
            "pivot.stageb.native_patch_category_d8_source_audit/v1", source
        )
