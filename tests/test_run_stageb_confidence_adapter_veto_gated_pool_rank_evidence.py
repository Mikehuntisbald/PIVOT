from tools import (
    run_stageb_confidence_adapter_veto_gated_pool_rank_evidence_probe as probe,
)
from tools import (
    run_stageb_confidence_adapter_veto_gated_pool_rank_evidence_probe_u0050 as probe_u0050,
)
from util.stage_b_dense_duty_audit import build_training_contract


def test_rank_evidence_v11_probe_is_fresh_and_binds_u6551():
    ...
    assert contract["schema"] == (
        "pivot.stageb.dense_duty_training_contract/v13"
    )
...
def test_rank_evidence_v11_u300_uses_independent_output():
    command = probe.command("start")
    assert "--resume" not in command
    assert command[command.index("--max_train_iters") + 1] == "300"
    assert "dense_duty_adapter_veto_gated_pool_rank_evidence_highmem_20260731" in str(
        probe.OUTPUT
    )
