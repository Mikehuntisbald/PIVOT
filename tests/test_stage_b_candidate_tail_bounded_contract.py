...
from tools import (
    run_stageb_confidence_adapter_candidate_tail_bounded_probe_evaluation as evaluation,
)
from tools import (
    run_stageb_confidence_adapter_candidate_tail_bounded_probe_u0400 as probe,
)
...
PROBE_CONFIG = REPO_ROOT / (
    "config/ablations/"
    "cfg_stageb_dense_duty_confidence_adapter_candidate_tail_bounded_"
    "probe_u0400_20260801.py"
)
FORMAL_CONFIG = REPO_ROOT / (
    "config/ablations/"
    "cfg_stageb_dense_duty_confidence_adapter_candidate_tail_bounded_20260801.py"
)


def test_tail_bounded_config_and_training_contract_are_bound():
...
def test_tail_bounded_probe_and_promotion_are_isolated_and_fixed():
...
def test_tail_bounded_probe_disables_formal_admission():
    cfg = SLConfig.fromfile(str(PROBE_CONFIG))
    assert _bind_stage_b_confidence_probe_admission(cfg) is None


def test_tail_bounded_formal_binding_fails_closed_until_report_exists():
    cfg = SLConfig.fromfile(str(FORMAL_CONFIG))
    report = Path(cfg.stage_b_dense_duty_confidence_probe_admission_report)
    if report.exists():
        pytest.skip("a completed v37 probe report is already present")
