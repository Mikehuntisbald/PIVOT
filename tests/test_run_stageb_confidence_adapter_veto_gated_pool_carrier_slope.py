from tools import (
    run_stageb_confidence_adapter_veto_gated_pool_carrier_slope_probe as probe,
)
from tools import (
    run_stageb_confidence_adapter_veto_gated_pool_carrier_slope_probe_u0050 as probe_u0050,
)
from util.stage_b_dense_duty_audit import build_training_contract


def test_carrier_slope_v14_probe_is_fresh_and_binds_u6551():
    ...
    assert values["stage_b_v11_trainable_params_min"] == 185_989
    assert values["stage_b_v11_trainable_params_max"] == 185_989
    assert values["stage_b_dense_duty_rank_source_optimizer_updates"] == 6551

    contract = build_training_contract(values)
    assert contract["schema"] == "pivot.stageb.dense_duty_training_contract/v16"
...
def test_carrier_slope_v14_u300_uses_independent_output():
    command = probe.command("start")
    assert "--resume" not in command
    assert command[command.index("--max_train_iters") + 1] == "300"
    assert "dense_duty_adapter_veto_gated_pool_carrier_slope_highmem_20260731" in str(
        probe.OUTPUT
    )
