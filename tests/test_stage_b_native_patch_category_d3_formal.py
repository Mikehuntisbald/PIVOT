...
FORMAL_CONFIG = (
    REPO_ROOT
    / "config/ablations/cfg_stageb_native_patch_category_d3_u200.py"
)
SMOKE_CONFIG = (
    REPO_ROOT
    / "config/ablations/cfg_stageb_native_patch_category_d3_smoke.py"
)
DATASET_CONFIG = (
    REPO_ROOT
...
def _d3_runtime_args(root: Path) -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        stage_b_native_patch_execution_scope=_STAGE_B_NATIVE_PATCH_D3_U200_SCOPE,
        ...
        stage_b_native_patch_d3_base_initializer_path=str(base_initializer),
        stage_b_native_patch_d3_base_initializer_sha256=_sha256(base_initializer),
...
class StageBNativePatchCategoryD3FormalTest(unittest.TestCase):
    def test_formal_and_smoke_configs_are_exact(self) -> None:
        ...
        self.assertEqual(
            ...
            REPO_ROOT
            / "outputs/paper_cvpr_v1/native_patch_category_d3_seed42_s43_b36a2_lr5e5_amp16_u200_v2",
        )
        self.assertEqual(_sha256(DATASET_CONFIG), cfg["stage_b_native_patch_dataset_config_sha256"])
    ...
    def test_d3_runtime_binding_is_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            args = _d3_runtime_args(Path(directory))
            with patch.dict(
                os.environ,
            ...
            self.assertEqual(
                binding["schema"],
                "pivot.stageb.native_patch_category_d3_runtime/v1",
            )
            self.assertEqual(binding["runtime"]["max_train_iters"], 200)
            self.assertEqual(binding["runtime"]["iter_checkpoint_interval"], 50)
            self.assertIn("d3_base_initializer", binding)
            self.assertNotIn("d2_base_initializer", binding)
            ...
            for key, bad_value, message in (
                ...
                (
                    "stage_b_native_patch_d3_critical_weight",
                    1.0,
                    "critical-winner objective drifted",
                ),
                ("amp_init_scale", 64.0, "critical-winner objective drifted"),
            ):
    ...
    def test_d3_strict_resume_uses_200_update_contract_and_d1_lineage(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_pretrain_route_audits_d1_as_d3_source(self) -> None:
        source = inspect.getsource(training_main.main)
        self.assertIn('"d3_critical_winner"', source)
        self.assertIn("stage_b_native_patch_d3_base_initializer_path", source)
        self.assertIn("audit_d2_source_transition", source)
        self.assertIn("stage_b_native_patch_d3_source_audit", source)
