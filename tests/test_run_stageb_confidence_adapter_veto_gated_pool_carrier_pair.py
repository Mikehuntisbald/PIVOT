from tools import (
    audit_stageb_confidence_adapter_veto_gated_pool_carrier_pair_probe_health as health,
)
from tools import (
    run_stageb_confidence_adapter_veto_gated_pool_carrier_pair_highmem_formal as formal,
)
from tools import (
    run_stageb_confidence_adapter_veto_gated_pool_carrier_pair_probe as probe,
)
from tools import (
    run_stageb_confidence_adapter_veto_gated_pool_carrier_pair_probe_evaluation as promotion,
)
from tools import (
    run_stageb_confidence_adapter_veto_gated_pool_carrier_pair_probe_u0050 as probe_u0050,
)
from util.stage_b_dense_duty_audit import validate_formal_invocation
...
def test_carrier_pair_formal_contract_binds_v9_pair_and_u6551(monkeypatch):
    monkeypatch.setattr(
        formal._BASE,
    ...
    training_contract = formal._BASE.build_training_contract(values)
    assert training_contract["schema"] == (
        "pivot.stageb.dense_duty_training_contract/v11"
    )
    assert training_contract["values"][
        "stage_b_dense_duty_raw_veto_carrier_pair_weight"
    ] == 0.25
    assert training_contract["values"][
        "stage_b_dense_duty_raw_veto_carrier_pair_margin"
    ] == 0.25
    validate_formal_invocation(values, repo_root=formal.REPO_ROOT)


def test_carrier_pair_probe_and_promotion_are_fresh_exact_contracts():
    ...
    assert "dense_duty_adapter_veto_gated_pool_carrier_pair_highmem_20260731" in (
        str(probe.OUTPUT)
    )
    assert promotion.SCHEMA.endswith("/v9")
    assert promotion.POSTFLIGHT_SCHEMA.endswith("/v9")
    assert promotion.ADMISSION_SCHEMA.endswith("/v9")
    assert promotion.HEALTH_SCHEMA == health.SCHEMA
    assert health.SCHEMA.endswith("/v9")
...
def _healthy_trajectory():
    return {
        ...
        "train_fixed_text_raw_veto_tn_balanced_loss_mean_unscaled": 0.035,
        ...
        "train_loss_fixed_text_raw_veto_carrier_pair_unscaled": 0.01,
        "train_fixed_text_raw_veto_carrier_pair_sample_count_unscaled": 15.5,
        "train_fixed_text_raw_veto_carrier_pair_gap_mean_unscaled": 0.30,
        "train_fixed_text_raw_veto_carrier_pair_hinge_mean_unscaled": 0.01,
        "train_fixed_text_raw_veto_carrier_pair_violation_rate_unscaled": 0.10,
    }


def test_carrier_pair_health_preregisters_routing_pairing_and_attribution(
    monkeypatch,
):
    monkeypatch.setattr(
        health,
    ...
    wrong_mix["train_fixed_text_raw_veto_tn_balanced_loss_mean_unscaled"] = 0.03
    checks = health._health_checks(wrong_mix, {}, {})
    assert not checks["u222_tn_balance_exact"]["passed"]

    changed_positive_loss = _healthy_trajectory()
    changed_positive_loss[
        "train_fixed_text_raw_veto_positive_loss_mean_unscaled"
    ] = 0.04
    checks = health._health_checks(changed_positive_loss, {}, {})
    assert not checks["u222_positive_loss_unchanged"]["passed"]

    missing_pair = _healthy_trajectory()
    missing_pair[
        "train_fixed_text_raw_veto_carrier_pair_sample_count_unscaled"
    ] = 15.0
    checks = health._health_checks(missing_pair, {}, {})
    assert not checks["u222_carrier_pair_support_complete"]["passed"]

    weak_pair = _healthy_trajectory()
    weak_pair["train_fixed_text_raw_veto_carrier_pair_gap_mean_unscaled"] = 0.24
    weak_pair["train_fixed_text_raw_veto_carrier_pair_hinge_mean_unscaled"] = 0.03
    weak_pair["train_loss_fixed_text_raw_veto_carrier_pair_unscaled"] = 0.03
    weak_pair[
        "train_fixed_text_raw_veto_carrier_pair_violation_rate_unscaled"
    ] = 0.25
    checks = health._health_checks(weak_pair, {}, {})
    assert not checks["u222_carrier_pair_gap_reaches_margin"]["passed"]
    assert not checks["u222_carrier_pair_hinge_small"]["passed"]
    assert not checks["u222_carrier_pair_violation_bounded"]["passed"]
