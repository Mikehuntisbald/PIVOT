...
from tools import (
    run_stageb_confidence_adapter_candidate_hardest_edit_snapshot_screen
    as screen,
)
...
HARD_SCOPE = "target_iou_union_detached_final_confidence_base_argmax_v2"
...
def _args(update: int = 100, **overrides):
    values = {
        ...
        "config": str(CONFIG),
        "output_dir": str(
            combined_eval._V40_IMMUTABLE_ARCHIVED_SCREEN_OUTPUTS[update]
        ),
        "ckpts": [
            str(combined_eval._V40_IMMUTABLE_ARCHIVED_SNAPSHOT_PATHS[update])
        ],
        "tn_jsonl": str(
...
def _saved_args():
    terminal = Path(ref_eval._V40_IMMUTABLE_ARCHIVED_TERMINAL["path"])
    return {
        ...
        "stage_b_v21_token_edit_query_scope": HARD_SCOPE,
    }


def test_v40_snapshot_allowlist_and_wrapper_command_are_exact():
    assert set(ref_eval._V40_IMMUTABLE_ARCHIVED_SNAPSHOTS) == {100, 200, 300}
    assert {
        update: spec["sha256"]
        for update, spec in ref_eval._V40_IMMUTABLE_ARCHIVED_SNAPSHOTS.items()
    } == {
        100: "a97b63b94cc5391a92a58a202ef3d4239b792fe6694747ad2e72a593be372b88",
        200: "261fee3242a5b93067e8c8f04d1f7d79ef54a0cc6897389a920bc8cf7b378e8f",
        300: "71c96eef83e780f71d39f67502d47b975b3799ae89e080ed23de06f77e700da4",
    }
    assert ref_eval._V40_IMMUTABLE_ARCHIVED_TERMINAL["sha256"] == (
        "898f2be8545b71dd8ba0ee2d0d2f7a669d3680b95d1952bfe3ccd5f2e905dc00"
    )
    command = screen.build_command(200)
    assert "--partial_dense_duty_confidence_diagnostic" in command
    assert "--immutable_v40_archived_snapshot_diagnostic" in command
...
def test_v40_immutable_cli_requires_existing_partial_diagnostic_flag():
    cfg = SLConfig.fromfile(str(CONFIG))
    with pytest.raises(ValueError, match="partial_dense_duty_confidence"):
        combined_eval._validate_immutable_v40_archived_snapshot_diagnostic_args(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"output_dir": "/tmp/not-the-fixed-v40-screen"}, "fixed immutable"),
...
def test_v40_immutable_cli_rejects_scope_path_and_runtime_drift(
    overrides, message
):
    cfg = SLConfig.fromfile(str(CONFIG))
...
def test_v40_immutable_cli_contract_accepts_only_the_fixed_surface():
    cfg = SLConfig.fromfile(str(CONFIG))
    combined_eval._validate_immutable_v40_archived_snapshot_diagnostic_args(
        _args(300), cfg
    )
...
def test_v40_snapshot_metadata_rejects_update_reason_and_scope_drift():
    ...
    ref_eval._validate_v40_immutable_archived_snapshot_metadata(
        payload,
    ...
    drifted = copy.deepcopy(payload)
    drifted["args"]["stage_b_v21_token_edit_query_scope"] = "target_iou_v1"
    with pytest.raises(RuntimeError, match="directory/update/reason"):
        ref_eval._validate_v40_immutable_archived_snapshot_metadata(
            drifted,
            checkpoint_path=checkpoint,
...
def test_v40_terminal_metadata_requires_u400_and_hardest_edit_scope():
    terminal = Path(ref_eval._V40_IMMUTABLE_ARCHIVED_TERMINAL["path"]).resolve(
        strict=True
    )
    payload = {
    ...
    ref_eval._validate_v40_immutable_archived_terminal_metadata(
        payload, checkpoint_path=terminal
    )
    payload["args"]["stage_b_v21_token_edit_query_scope"] = "target_iou_v1"
...
def test_v40_file_verifier_rejects_snapshot_and_terminal_sha_drift(monkeypatch):
    monkeypatch.setattr(ref_eval, "_sha256_file", lambda _path: "0" * 64)
    with pytest.raises(RuntimeError, match="snapshot changed"):
        ref_eval._verify_v40_immutable_archived_diagnostic_files(
            screen.SNAPSHOTS[100]
        )


def test_v40_immutable_file_record_rejects_symlink(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="must not be symlinks"):
        ref_eval._v40_immutable_archived_file_record(symlink)


def test_v40_summary_provenance_uses_only_the_v40_flag(tmp_path, monkeypatch):
    ...
    cfg = SimpleNamespace(
        ...
        stage_b_dense_duty_immutable_v40_archived_snapshot_diagnostic=True,
    ...
    assert result["immutable_v40_archived_snapshot_diagnostic"] is True
...
def _summary_row(update: int):
    snapshot = {
        ...
        "size_bytes": 1,
        "sha256": ref_eval._V40_IMMUTABLE_ARCHIVED_SNAPSHOTS[update]["sha256"],
    }
    terminal_path = Path(ref_eval._V40_IMMUTABLE_ARCHIVED_TERMINAL["path"])
    terminal = {
        "path": str(terminal_path.resolve(strict=True)),
        "size_bytes": 1,
        "sha256": ref_eval._V40_IMMUTABLE_ARCHIVED_TERMINAL["sha256"],
    }
    provenance = {
        "schema": "pivot.stageb.v40_immutable_archived_diagnostic/v1",
...
def test_v40_summary_verifier_never_accepts_v39_formal_or_terminal_evidence(
    ...
    assert "immutable_v39_archived_snapshot_diagnostic" not in accepted
